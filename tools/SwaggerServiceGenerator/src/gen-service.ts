...
const getSwaggerData = async (baseUrl: string) => {
  ...
  const jsonText = JSON.stringify(response.data).replaceAll(
    "#/components/schemas/",
    ""
  );
...
const transformToCamelCase = (name: string): string => {
  ...
  return `${controller.charAt(0).toLowerCase()}${controller.substring(
    1
...
const generateModels = (
  allEndpoints: { path: string; verb: string; data: EndpointDto }[]
) => {
  const models = allEndpoints.map((e) => {
    ...
    return [responseType, body] as string[];
  });
...
export class ${serviceName.charAt(0).toUpperCase()}${serviceName.substring(
    1
  )}Service {
...
export const generate = async (
  ...
  const methodCalls = allEndpoints.map((e) => {
    ...
    const methodParameters = e.data.parameters.map(
      (p) => `${p.name}${!p.required ? "?" : ""}: ${p.schema.type}`
    );
    ...
    return `${method} async (${methodParameters.join(
      ", "
    )}): ${response} => this.apiService.${
      e.verb
    }${apiCallResponse}(${apiCallParameters.join(", ")});`;
  });
  ...
  const serviceContent = generateService(
    serviceName,
    `import { ${models.join(", ")} } from '../generated-models';`,
    methodCalls
  );
